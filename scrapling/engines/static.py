...
class StaticEngine:
    ...
    def _prepare_response(self, response: httpxResponse) -> Response:
        ...
        :return: A `Response` object that is the same as `Adaptor` object except it has these added attributes: `status`, `reason`, `cookies`, `headers`, `history` and `request_headers`
        """
        return Response(
